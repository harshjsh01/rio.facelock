...
Item {
  ...
  function refreshFaceStatus() {
    ...
    if (!weeklyPasswordCheckProc.running) weeklyPasswordCheckProc.running = true
  }
  ...
  function recordPasswordUnlock() {
    weeklyPasswordRequired = false
    if (!updatePasswordTimestampProc.running) updatePasswordTimestampProc.running = true
  }
  ...
  function startFaceUnlock() {
    if (!lockRequested || !sessionLock.secure || !faceUnlockConfigured) return
    if (weeklyPasswordRequired) {
      logEvent("face-auth-skipped: weekly password required")
      return
    }
  ...
  function handleFaceFinished(result) {
    ...
    if (result === PamResult.Success) {
      ...
      faceState = "failed"
      if (faceUnlockConfigured && !weeklyPasswordRequired) {
  ...
  WlSessionLock {
    ...
    onSecureStateChanged: {
      ...
      if (secure) {
        ...
        if (!root.weeklyPasswordRequired) {
          root.startFaceUnlock()
        }
      }
    }
    ...
    WlSessionLockSurface {
      ...
      LockView {
        ...
        faceUnlockConfigured: root.faceUnlockConfigured
        faceState: root.faceState
        weeklyPasswordRequired: root.weeklyPasswordRequired
  ...
  Timer {
    ...
    repeat: false
    onTriggered: {
      if (root.lockRequested && root.faceUnlockConfigured && !root.weeklyPasswordRequired) {
        root.startFaceUnlock()
      }
    }
  }
  ...
  Process {
    id: readlinkProc
    command: ["bash", "-c", "if [ -f \"$HOME/.local/state/omarchy/current/theme/unlock.png\" ]; then echo \"$HOME/.local/state/omarchy/current/theme/unlock.png\"; else readlink -f \"$HOME/.local/state/omarchy/current/background\"; fi"]
  ...
  Process {
    id: updatePasswordTimestampProc
    command: ["bash", "-c", "mkdir -p \"$HOME/.local/state/omarchy\" && date +%s > \"$HOME/.local/state/omarchy/last_password_unlock\""]
  }

  Process {
    id: weeklyPasswordCheckProc
    command: ["bash", "-c", "FILE=\"$HOME/.local/state/omarchy/last_password_unlock\"; if [ ! -f \"$FILE\" ]; then mkdir -p \"$(dirname \"$FILE\")\"; date +%s > \"$FILE\"; echo \"ok\"; else LAST=$(cat \"$FILE\" 2>/dev/null || echo 0); NOW=$(date +%s); DIFF=$(( NOW - LAST )); if [ $DIFF -ge 604800 ]; then echo \"required\"; else echo \"ok\"; fi; fi"]
    stdout: StdioCollector { id: weeklyPasswordStdout; waitForEnd: true }
    onExited: {
      var res = String(weeklyPasswordStdout.text || "").trim()
      root.weeklyPasswordRequired = (res === "required")
      if (root.lockRequested && root.sessionLock.secure && root.faceUnlockConfigured && !root.weeklyPasswordRequired) {
        root.startFaceUnlock()
      }
    }
  }
  ...
  Process {
    id: faceCheckProc
    command: ["bash", "-c", "if [[ -f /etc/pam.d/omarchy-lock-face ]] && command -v facelock >/dev/null 2>&1; then echo yes; else echo no; fi"]
    stdout: StdioCollector { id: faceCheckStdout; waitForEnd: true }
    onExited: {
      root.faceUnlockConfigured = String(faceCheckStdout.text || "").trim() === "yes"
      if (root.lockRequested && root.sessionLock.secure && root.faceUnlockConfigured && !root.weeklyPasswordRequired) {
        root.startFaceUnlock()
      } else if (!root.faceUnlockConfigured && facePam.active) {
        facePam.abort()
      }
    }
  }
  ...
  IpcHandler {
    ...
    function status(): string {
      return JSON.stringify({
        ...
        faceUnlock: root.faceUnlockConfigured,
        faceState: root.faceState,
        weeklyPasswordRequired: root.weeklyPasswordRequired,
